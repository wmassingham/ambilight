...
HRESULT DXGIOutputDuplication::AcquireNextFrame(IDXGISurface1** pDXGISurface, DXGIPointerInfo*& pDXGIPointer)
{
	...
	if(FAILED(hr))
	{
		if (hr != DXGI_ERROR_WAIT_TIMEOUT) { printf("m_DXGIOutputDuplication->AcquireNextFrame failed with hr=0x%08x\n", hr); };
		return hr;
	}
...
HRESULT DXGIManager::GetOutputBits(BYTE* pBits, RECT& rcDest)
{
		...
		iter++)
	{
		...
		hr = out.AcquireNextFrame(&spDXGISurface1, m_pDXGIPointer); // this line is failing
		if (FAILED(hr))	{
			if (hr != DXGI_ERROR_WAIT_TIMEOUT) { printf("failed to acquire next frame: 0x%08x\n", hr); }
